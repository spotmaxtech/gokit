...
// Simple template only percent
func NewSimple(total int) *Bar {
	bar := New(float64(total))
	bar.Template(`{{.Percent | printf "%3.0f"}}%`)
	return bar
}
...
// auto progressing string
func (b *Bar) AutoString() string {
	b.value += 1
	if b.value > b.Total {
		b.value = b.Total
	}
